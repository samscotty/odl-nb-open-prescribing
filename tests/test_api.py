...
SPENDING_BY_CCG_TEST_JSON_DATA = [
    CCGSpend(
        items=600,
        quantity=10000.0,
...
def test_query_spending_by_ccg(mock_query_spending_by_ccg_api_response):
    ...
    mock_query_spending_by_ccg_api_response.assert_called_once()
    assert response == SPENDING_BY_CCG_TEST_JSON_DATA
...
def test_query_org_location(mock_query_org_location_api_response):
    ...
    mock_query_org_location_api_response.assert_called_once()
...
def test_http_api_data_provider_get_chemical_spending_for_ccg(
    mock_query_spending_by_ccg_api_response,
):
    provider = HttpApiDataProvider()
    response = provider.chemical_spending_for_ccg(chemical="BADF00D", ccg="ABC")
    mock_query_spending_by_ccg_api_response.assert_called_once_with(
        path="spending_by_ccg", api_params={"code": "BADF00D", "org": "ABC"}
    )
    assert response == SPENDING_BY_CCG_TEST_JSON_DATA


def test_http_api_data_provider_gets_ccg_boundaries(mock_query_org_location_api_response):
    provider = HttpApiDataProvider()
    provider.ccg_boundaries()
    mock_query_org_location_api_response.assert_called_once_with(
